...
class InMemoryTaskManagerTest extends TaskManagerTest<InMemoryTaskManager> {
    ...
    void createTaskWithNotNewStatus() throws IOException {
        ...
        assertEquals(Status.NEW, manager.getTaskList().get(1).getStatus(),
                "Неверный статус задачи, ожидался: " + Status.NEW);
        assertEquals(Status.NEW, manager.getTaskList().get(2).getStatus(),
                "Неверный статус задачи, ожидался: " + Status.NEW);
    }
    ...
    void createEpicWithTwoSubtaskWithDoneStatus() throws IOException {
        ArrayList<Subtask> epicSubtasks = new ArrayList<>(List.of(subtaskEpic2_1_StatusDoneTest,
                subtaskEpic2_2_StatusDoneTest));
